...
<html lang="en">
  ...
  <body>
    ...
    <div class="jumbotron">
      <h1>lutline</h1>
      <p class="lead">Parse <code>argv</code> using a fast look-up table generated from a command-line interface specification.</p>
    </div>
...
with open("index.md") as f:
    index = f.read()
headings = [l[2:].strip() for l in index.splitlines() if l.startswith("##")]
tags = [h.lower().replace(" ", "-") for h in headings]
__t = '<li><a href="#{tag}">{heading}</a></li>'
toc = "\n".join(__t.format(tag=t, heading=h) for t, h in zip(tags, headings))
body = markdown2.markdown(index, extras=['fenced-code-blocks', 'toc'])
doc = TEMPLATE.format(toc=toc, body=body, date="This page was generated in " + datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
with open("index_.html", "w") as f:
    f.write(doc)
